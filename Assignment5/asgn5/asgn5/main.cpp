...
//****** Start of Class BinaryHeap ********
template <typename aEventRec>
class BinaryHeap
{
public:
	//Default constructor - with an optional capacity (that defaults to 100)
   //array is a Vector type and int currentSize are private member variable of the class
	explicit BinaryHeap(int capacity = 100)
		: array(capacity + 1), currentSize(0)//array[0] is left blank always
	{
	}
		...
		: array(items.size() + 10), currentSize(items.size())
	{
		...
		{
			array[i + 1] = items[i]; // vector items copied into vector array
		}
		buildHeap(); //private method - array elements are ordered in a heap structure
		            // using private percolateDown() method
	}
	...
	*/
	const aEventRec & getMin() const
	{
		...
		return array[1];//array[0] is blank and array[1] will always be  
	}                  // the min/max depending on min/max Binary heap created
	...
	*/
	...
	{
		if (currentSize == array.size() - 1)//if Binary heap is full then resized to double initial 
		{
...
private:
	...
	/* Establish heap order property from an arbitrary
	   ...
	   Basically rearranges the array elements in a binary heap form
	*/
	void buildHeap()
	{
		for (int i = currentSize / 2; i > 0; i--)//i = (currentSize/2) gives the parent of last item in array
		{
			percolateDown(i); //percolated down to its appropriate place
							 // starting from posn(i) = currentSize / 2 = hole
		}                   //i = (currentSize/2) gives the parent of last item in array
	}

	/**
	* Internal method to percolate down in the heap.
	* hole is the index at which the percolateDown begins.
	...
	*/
	...
	void percolateDown(int hole)   // hole = parent, & (hole*2) = child(first)
	{
		...
		for (; hole * 2 <= currentSize; hole = child) // hole = child means child loc of previous iteration becomes the
		...
		{                                            //till 
			...
			if (child != currentSize && array[child + 1] < array[child])
			{
				child++; // choose the index of the lower of the two child
				        // i.e. here now right child chosen
			}//else child = left child chosen

			if (array[child] < tmp) // if child element is less than the tmp = array[hole](parent) value that came in
			{
				array[hole] = array[child];// child element is moved up
										   // to its parent loc(current hole loc)
			}
			else//case when neither child is greater than parent
			{
				break; //case when both child are greater than parent
			}
		}
		array[hole] = tmp; // then here tmp = original array[hole](parent)  value is stored at the new hole loc
	}
};

//****** End of Class BinaryHeap ********

//****** Start of Class StackType : ProtoType ********
template <class Type>
class StackType
{
public:
...
//****** End of Class StackType : ProtoType ********

//****** Start of Class StackType : Implementation ********
...
//****** End of Class StackType : Implementation ********
...
int main()
{
	...
	int hour = 0;
	int noOfFltsPerHr = NO_OF_FLIGHTS;  //  14; // sizeof(IN_FLTS_HOURLY_SCHEDULE); //.size();// holds the number of flights per hour initially declared
	...
	{
		aFltRec = flightsHeap1.getMin(); // to get time of the root record(next for dep) 		
...
// Number of gates entered by user is used to create the stack of gate numbers
void createGateNosStack(StackType<int> &aGateList, int noOfGates)
{
	for (int i = 1; i <= noOfGates; i++)
	{
		aGateList.push(i);//a List of gate Nos is created
	}
}
...
//		{
//			aEventRec aFltRec;
//			aFltRec = depFlightsHeap.getMin(); // to get time of the root record(next for dep) 
//			while (currArrTime > aFltRec.arrDepTime)
//			{
//				gateNosStack.push(aFltRec.gateNo);	//get the gate no of root record that is to be deleted and replenish the gateStack				
//				depFlightsHeap.deleteMin(); // delete the root record as would have departed at curr time
//				aFltRec = depFlightsHeap.getMin();// get the next new root record(next for dep) for next iteration					
//			}
//			gateNosStack.invertStack(); // invert stack to prevent last in first out
//		}
